...
class LinearRegression:
    ...
    def get_regularized_cost(self, X: np.array, y: np.array, w: np.array, b: float):
        ...
        cost += (self.l / m) * np.sum([w_i ** 2 for w_i in w])
        return cost
    ...
    def compute_gradient_descent(self, X: np.array, y: np.array, w: np.array, b: float, eps: float):
        ...
        while np.abs(prev_cost - current_cost) > eps:
            dj_dw, dj_db = self.get_gradient_descent_step(X, y, current_w,
                                                          current_b) if self.l == 0 else self.get_regularized_gradient_descent_step(
                X, y, w, b)
...
if __name__ == '__main__':
    X_train = np.array([[4.17022005e-01, 7.20324493e-01, 1.14374817e-04],
    ...
    lr = LinearRegression(alpha, lambda_initial)
